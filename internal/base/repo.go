...
func (r *Repo) CopyTo(ctx context.Context, to string, modPath string, ignores []string) error {
	...
	if err := copyDir(r.Path(), to, []string{
		mod, modPath,
		strings.ToUpper(mod[:1]) + mod[1:], strings.ToUpper(modPath[:1]) + modPath[1:]}, ignores); err != nil {
		return err
	}

	apiPath := path.Dir(path.Dir(to)) + "/api"
	if _, err := os.Stat(apiPath); err == nil {
		defer os.RemoveAll(to + "/api")

		log.Println("debug:", r.Path()+"/api", apiPath)
		return copyDir(r.Path()+"/api", apiPath, []string{
			mod, modPath,
			strings.ToUpper(mod[:1]) + mod[1:], strings.ToUpper(modPath[:1]) + modPath[1:]}, ignores)
	}
